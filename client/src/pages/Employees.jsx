...
import { FiUserPlus, FiTrash2, FiEdit2, FiUser, FiPhone, FiX, FiSave, FiKey } from 'react-icons/fi';
...
const STYLE = `
  ...
  .modal-box { background: white; border-radius: 20px; padding: 28px; max-width: 460px; width: 100%; }
...
export default function Employees() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', employeeRole: 'receptionist', password: '' });
  ...
  const openAdd = () => { setForm({ name: '', phone: '', employeeRole: 'receptionist', password: '' }); setEditingId(null); setShowModal(true); };
  const openEdit = (emp) => { setForm({ name: emp.name, phone: emp.phone, employeeRole: emp.employeeRole || 'receptionist', password: '' }); setEditingId(emp._id); setShowModal(true); };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        const updates = { name: form.name, phone: form.phone, employeeRole: form.employeeRole };
      ...
      <div className="emp-page">
        <div className="emp-header">
          <div>
            ...
            <p style={{ color: '#64748b', fontSize: 13, margin: '4px 0 0' }}>إضافة وإدارة موظفي العيادة</p>
          </div>
            ...
            {employees.map(emp => {
              ...
              return (
                <div key={emp._id} className="emp-card">
                  ...
                  <div style={{ background: '#f8fafc', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#2563eb', fontWeight: 700 }}>
                      <FiKey size={11} /> بيانات الدخول
                    </div>
                    <div style={{ color: '#475569', marginTop: 4 }}>رقم الجوال: <strong>{emp.phone}</strong></div>
                    <div style={{ color: '#64748b', fontSize: 11, marginTop: 2 }}>كلمة المرور التي تم إعدادها عند الإنشاء</div>
                  </div>
                  {emp.createdAt && (
